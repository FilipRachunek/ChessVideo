...
public class Move {
    ...
    private Color color;
    private Type type, promoteType;
    protected int fromRow = -1, fromColumn = -1, toRow = -1, toColumn = -1;
    private boolean check, castlingShort, castlingLong, capture, checkmate, relayed, ambiguous, place;
    private String screenshotId;

    public Move() {
    }

    public Move(Game game, int moveNumber, String pgnCode, Color color) {
        this.moveNumber = moveNumber;
        this.pgnCode = pgnCode;
        this.color = color;
        // check screenshot ID
        if (pgnCode.endsWith("]")) {
            int index = pgnCode.indexOf("[");
            screenshotId = pgnCode.substring(index + 1, pgnCode.length() - 1);
            pgnCode = pgnCode.substring(0, index);
            this.pgnCode = pgnCode;
        }
        // get piece type and target square
        String[] array = pgnCode.split("");
        ...
        if (pgnCode.startsWith("?-")) {
            ...
            } else if (index > 0 && array[index].equals("x")) {
                // after removing the target square, the move could end with a capture sign
                capture = true;
                index--;
                int fromColumnIndex = getColumnIndex(array[index]);
                int fromRowIndex = getRowIndex(array[index]);
    ...
    public static Move from(Move from) {
        Move move = new Move();
    ...
    private int getRowIndex(String rowCode) {
        for (int i = 0; i < 10; i++) {
